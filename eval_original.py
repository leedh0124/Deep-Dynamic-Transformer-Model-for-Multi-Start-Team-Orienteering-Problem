...
from utils_original import load_model, move_to
from utils_original.data_utils import save_dataset
from torch.utils.data import DataLoader
import time
from datetime import timedelta
from utils_original.functions import parse_softmax_temperature
from utils.functions import augment_xy_data_by_8_fold
mp = torch.multiprocessing.get_context('spawn')

# 0425: Add x8 instance augmentation
...
def _eval_dataset(model, dataset, width, softmax_temp, opts, device):
    ...
    model.set_decode_type(
        "greedy" if opts.decode_strategy in ('bs', 'greedy', 'inst_aug') else "sampling",
        temp=softmax_temp)
    ...
    for batch in tqdm(dataloader, disable=opts.no_progress_bar):
        ...
        with torch.no_grad():
            if opts.decode_strategy in ('sample', 'greedy'):
                ...
                aug_factor = 8 # default 
                batch_rep = 1
                iter_rep = 1
                if model.problem.NAME == 'tsp':
                    batch = augment_xy_data_by_8_fold(batch)
                elif model.problem.NAME == 'cvrp':
                    batch['depot'] = augment_xy_data_by_8_fold(batch['depot'][:,None]).squeeze(1)
                    batch['loc'] = augment_xy_data_by_8_fold(batch['loc']) # [1024*8, 50, 2]
                    batch['demand'] = batch['demand'].repeat(aug_factor, 1) # [1024*8, 50]
                else: 
                ...
                costs, max_reward_idx = aug_reward.min(dim=0) 
...
if __name__ == "__main__":
    ...
    parser.add_argument("datasets", default=['data/tsp/tsp50_test_seed1234.pkl']) # uncomment for inst_aug
    ...
    parser.add_argument('--decode_strategy', type=str,
                        help='Beam search (bs), Sampling (sample), Greedy (greedy) or , Instance Augmentation (inst_aug)')
    ...
    # Change opts (only for inst_aug)
    # opts.decode_strategy = 'inst_aug'
    
    if opts.decode_strategy == 'inst_aug': # uncomment for inst_aug
        opts.multiprocessing = False # one GPU available
        opts.datasets = ['data/tsp/tsp100_test_seed1234.pkl'] # ['data/vrp/vrp100_test_seed1234.pkl']
