...
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is None:
            if self.problem.NAME == 'top':
                self.dataset = self.problem.make_dataset(
                    size=self.opts.graph_size, num_veh=self.opts.num_veh, num_samples=self.opts.val_size, distribution=self.opts.data_distribution)
            else:
                self.dataset = self.problem.make_dataset(
    ...
    def eval(self, x, c):        
        if isinstance(self.model, torch.nn.DataParallel):
            assert (self.model.module.decode_type == 'greedy')
            assert (self.model.module.training == False)
        else:
            assert (self.model.decode_type == 'greedy')
